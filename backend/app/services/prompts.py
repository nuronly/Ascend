...
# ★ 第一批（快批）：只要最外围的几个前置概念，用小模型抢时间。
#   慢批（上面那份完整地图）实测能想一百多秒 —— 哪怕思考过程全程可见，
#   让人等一百秒才答第一道题也是不可接受的。而「学这个主题需要什么通识底子」
#   本来就不需要深思，正好交给小模型秒出，让用户立刻有题可答。
CALIBRATE_QUICK_SYSTEM = """你要快速列出学习某个主题**最外围的前置基础**。
