...
def note_routes_block(routes: list[dict]) -> str:
    """他前面写过笔记的小节清单 —— 记忆工具的「路由层」。

    ★ 为什么要在 prompt 里列清单，而不是让模型自己去 search_memory

      每次工具调用都是一轮完整的模型往返（正文一轮十几秒）。而「哪几节有笔记」
      是一句 SQL，几百字就列完了。所以：**发现靠清单，引用才靠工具**。
      模型看着清单直接决定读哪一节，省掉「先摸索一遍」的那一轮。

    ★ 没有笔记时也要明确说出来

      留空的话模型会自作主张编一条「与前面学过的内容相关」。宁可告诉它
      「他前面一份笔记都没有，这一节省略」。
    """
    if not routes:
        return (
            "【他前面还没有写过任何笔记】\n"
            "所以不要写「与前面学过的关系」——你手里没有任何依据，"
            "编一句关系比省略这一节糟得多。"
        )

    lines = ["【他前面几节写过的笔记（可用 read_note 读全文）】"]
    for r in routes:
        mark = "★他亲手改写过" if r.get("edited") else "AI 草稿，他还没动手"
        line = f"- {r.get('title', '')}（section_id={r.get('section_id', '')}，{mark}）"
        if concepts := [str(c) for c in (r.get("concepts") or [])]:
            line += f"\n    涉及：{'、'.join(concepts)}"
        lines.append(line)
    # 亲手改写过的那几节最值得读：那里有他自己的话
    lines.append(
        "标了「亲手改写过」的最值得读 —— 那一节里有他自己写下的理解，"
        "顺着他的说法往下讲，他会觉得这是接着自己的思路。"
    )
    return "\n".join(lines)
...
【他不是一张白纸 —— 讲之前先弄清他手里已经有什么】
这位学习者在本平台学过的每一节、写过的每一份笔记，你都可以查：
- read_note(section_id)：读他某一节笔记的当前全文。下面会列出他写过笔记的
  小节 —— 只要本节要承接其中某一节，就去读一遍：你能看到他**当时**是怎么
  理解的、哪里还卡着。然后接着他自己的话往下讲。
- my_boundary：他已掌握 / 半懂 / 明确说过没接触过的概念。已掌握的直接拿来用，
  说过没接触过的必须先铺垫。
- search_memory(query)：他在别的课里学过的东西，可以直接拿来打类比
  （「你在信号处理那门课里算过的卷积，就是这里的…」）。

宁可多查一次也不要凭空假设他的水平：把他已经会的东西再讲一遍，
他会觉得被当成傻子；用他没学过的词往下讲，他会当场卡住 —— 两种都会让他关掉页面。
查到的东西要真的用上，不要查完还是讲一份通用教程。
...
def section_user(
    *,
    ...
    if prereq_titles:
        # ★ 前置依赖是整套系统里唯一结构化的知识顺序（大纲阶段就定下的），
        #   比「已学过的小节」精确得多：那是一串标题，这是「这一节踩在谁身上」。
        #   讲之前必须先假定这几节他已经懂了，否则整节会退化成重讲一遍基础
        parts.append(
            f"本节的前置（大纲里已定，这几节的内容可以直接引用、不要重讲）："
            f"{'；'.join(prereq_titles[:6])}"
        )
    if prev_titles:
        # 只给标题不给正文：让模型知道讲过什么以避免重复，同时控制上下文成本
        parts.append(f"学习者已学过的小节（不要重复讲）：{'；'.join(prev_titles[-12:])}")
    if prior_notes:
        # 有笔记的小节才列 —— 没笔记时这一段整块不出现（正文不像笔记那样
        # 有「与前面的关系」这个必填节，不需要专门告诉它「他没写过笔记」）
        parts.append(note_routes_block(prior_notes))
...
# ─────────────────────────────────────────────────────────────
# 笔记卡（一节学完，卡片与原文汇流成的永久笔记）
# ─────────────────────────────────────────────────────────────
NOTE_SYSTEM = """你在帮学习者把刚学完的一节收成**一张笔记卡**。

这不是给别人看的教程摘要，是**他自己的**永久笔记 —— 半年后他重读这一张，
应该能立刻回到今天的理解水平。他会在你写的基础上亲手修改、补充、删减，
所以你的任务是给他一份**足够好的草稿**，不是一份不容置疑的成品。

【最重要的一条】
每一句都必须**具体到能被反驳**。禁止出现这类空话：
  ✗「本节介绍了注意力机制的基本原理」
  ✗「理解这一概念对后续学习很重要」
  ✓「注意力用点积算相似度，除以 √d 是为了让方差不随维度膨胀，
     否则 softmax 会饱和成 one-hot」
判断标准很简单：如果一句话换成任何别的主题也成立，它就是废话，删掉。

【他自己写过的话，原样保留】
凡是标注为「学习者己见」的内容，**逐字照抄**进「我的理解」那一节，
不要润色、不要改写、不要合并。那是他的思考，不是你的素材。
如果他一句都没写，「我的理解」就留空。

【必须留白】
「我的理解」和「我还没搞懂的」两节：
- 若有他的己见/存疑痕迹，放进去；
- 否则**只写标题，下面留空**。不要替他编内容，不要写「（待补充）」之类的占位话。
这两处空白是留给他动手的位置，比你多写十句更有价值。

【他前面写过的笔记，可以真的去读】
下面会列出他在这门课前面几节写过笔记的小节。你有两件工具：
- read_note(section_id)：读那一节笔记的**当前全文**（他改写过的版本）。
- search_memory(query)：在他全部学习记录里检索一个概念（含别的课）。

用途只有一个 —— 把「与前面学过的关系」写实。可以这样判断要不要读：
本节的核心概念里，有没有哪一个是前面某节的延续、对照或前提？有就去读一读，
看他**当时**是怎么理解的，然后点名写出关系（「这就是 1.2 里那个缩放因子，
只是换到多头之后每个头各算一遍」）。
读不出确切关联就整节省略。空泛的「与前文密切相关」比不写更糟：
它占着位置，却让他半年后完全想不起来到底什么关系。

【输出格式】
直接输出 Markdown，从 `##` 起（页面已有标题），严格按这个骨架，
没有内容的整节可以省略（但「我的理解」「我还没搞懂的」两节永远保留）：

## 这一节解决了什么问题
（1~2 句。要说清「不学这个会卡在哪」）

## 核心机制
（3~5 条，每条一句话，具体到能被反驳。有取舍的地方写出取舍）

## 我问过的关键问题
（只写他真的问过的，每条：问题 → 一句话结论。他没问过就省略这整节）
...
## 与前面学过的关系
（★ 必须查证过才能写：用 read_note 读过他前面某节的笔记，确认了具体关系
  再写 1~2 条，并点名是哪一节。没读到确切关联就省略这一节，别硬凑）

不要写「总结」「希望这份笔记对你有帮助」之类的收尾。
不要用一级标题。数学公式用 KaTeX（行内 $...$）。"""
...
def note_user(
    *,
    ...
    if key_concepts:
        parts.append(f"本节核心概念：{'、'.join(key_concepts)}")
    if unknown:
        # 开课校准时他说过这些不会 —— 笔记里尤其该讲清楚它们
        parts.append(
            f"他开课时说过这些概念没接触过（笔记里要确保覆盖到）：{'、'.join(unknown)}"
        )
    parts.append(note_routes_block(prior_notes or []))

    # 正文是笔记的主要素材，但要留出输出预算，长文截断
    parts.append(f"\n【本节正文】\n{content_md[:9000]}")

    if cards:
        parts.append("\n【他在这一节的提问与思考】")
        for i, c in enumerate(cards, 1):
            block = [f"{i}. 划中「{c.get('selected_text', '')}」，问：{c.get('question', '')}"]
            if ans := str(c.get("answer") or "").strip():
                block.append(f"   解答要点：{ans[:500]}")
            if note := str(c.get("user_note") or "").strip():
                # ★ 己见要原样进笔记，所以单独标出来，并在 system 里要求逐字照抄
                block.append(f"   ★ 学习者己见（必须逐字照抄）：{note[:800]}")
            parts.append("\n".join(block))
    else:
        parts.append("\n【他在这一节没有提问】所以「我问过的关键问题」这一节省略。")
